...
public class PageVO {
	...
	public PageVO(Criteria cri, int total) {
		...
		//end페이지계산
		//page가 1을 가르킬 때, end = 10 (보통 페이지는 1-10이기에)
		...
		//총 게시글 수가 232개라면 -> 마지막 번호 24
		//this.realEnd = (int) (Math.ceil(전체게시글 수 / 데이터 개수))
		this.realEnd = (int) (Math.ceil(this.total / (double) this.amount));
		
		//end페이지 재결정
		//데이터가 25개 -> end = 10, realEnd = 3 (실제 끝번호를 따라감)
		//데이터가 153개 (현재 조회하는 11번 페이지)-> end = 20, realEnd =1 6 (실제 긑번호를 따라감)
		//데이터가 153개 (현재 조회하는 3번 페이지) -> end = 10, realEnd = 16 (페이지네이션을 따라감)
		...
		//prev활성화 여부
		//start페이지 결정은 1, 11, 21, 31, 41..... 이런식으로 간다.
		this.prev = this.start > 1;
		
		//next활성화 여부
		//end = 10, realEnd = 16 라고 할 때 다음 버튼이 있다는 의미 
